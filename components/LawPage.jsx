...
const LawsPage = () => {
  ...
  const laws = [
    { id: 1, name: 'НИЙГМИЙН ДААТГАЛЫН ЕРӨНХИЙ ХУУЛЬ', date: '2023 оны 07 сарын 07 өдөр',type: '/Шинэчилсэн наруулга/', imageUrl: suldImage },
    { id: 2, name: 'НИЙГМИЙН ДААТГАЛЫН САНГААС ОЛГОХ ТЭТГЭВРИЙН ТУХАЙ', date: '2023 оны 07 сарын 07 өдөр', type: '/Шинэчилсэн наруулга/', imageUrl: suldImage },
    { id: 3, name: 'ДУНДАЖ ЦАЛИН ХӨЛС ТОДОРХОЙЛОХ ЖУРАМ', date: ' 2021 оны 12сарын 06 өдөр', imageUrl: suldImage },
    { id: 4, name: 'НИЙГМИЙН ДААТГАЛЫН ҮНДЭСНИЙ ЗӨВЛӨЛИЙН ТОГТООЛ', date: ' 2021 оны 12 сарын 27 өдөр', imageUrl: suldImage },
      ...
      <div className="flex flex-wrap justify-center ">
        {laws.slice(startIndex * lawsPerPage, (startIndex + 1) * lawsPerPage).map(law => (
          <div key={law.id} className="m-4 p-4 bg-white border justify-center border-gray-500 rounded-[20px] w-96">
            ...
            <div className="mt-4">
              <h3 className="font-semibold text-sm text-center mt-8">{law.name}</h3>
              <p className="text-gray-400 flex justify-center mt-4">{law.type}</p>
